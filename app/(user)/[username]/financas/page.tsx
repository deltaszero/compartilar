...
import type {
    ...
    SplitMethod,
    PeriodFilter,
} from "./components/types"; // import type { Friend, CostGroup, Expense, Balance, ExpenseMember, SplitMethod, PeriodFilter, FinanceUserData } from './components/types';
...
export default function FinancasPage() {
    ...
    useEffect(() => {
        console.log(
            "Effect for loading expenses triggered, selectedGroup:",
            selectedGroup
        );
        if (selectedGroup) {
    ...
    useEffect(() => {
        console.log(
            "Effect for reset percentages triggered, isAddingExpense:",
            isAddingExpense,
            "selectedGroup:",
            selectedGroup
        );
    ...
    useEffect(() => {
        console.log(
            "Effect for setting paid by triggered, isAddingExpense:",
            isAddingExpense,
            "userData:",
            !!userData
        );
    ...
    const handleCreateExpense = async (e: React.FormEvent) => {
        ...
        if (splitMethod === "percentage") {
            ...
            if (Math.abs(totalFixed - totalAmount) > 0.01) {
                toast.error(
                    `O total dos valores fixos deve ser igual ao valor total (${totalAmount})`
                );
                return;
            }
        }
    ...
    const handleDeleteExpense = async (expenseId: string) => {
        ...
        try {
            ...
            if (!isGroupCreator && !isPayer) {
                toast.error(
                    "Você só pode excluir despesas que você pagou ou se for o criador do grupo"
                );
                return;
            }
            ...
            toast.error(
                "Erro ao excluir despesa. Verifique se você tem permissão para esta ação."
            );
        }
    };
    ...
    const handleDeleteGroup = async (groupId: string) => {
        ...
        try {
            ...
            if (!userData || group.createdBy !== userData.uid) {
                toast.error(
                    "Você não tem permissão para excluir este grupo. Apenas o criador pode excluí-lo."
                );
                return;
            }
            ...
            toast.error(
                "Erro ao excluir grupo. Verifique se você tem permissão para esta ação."
            );
        }
    };
